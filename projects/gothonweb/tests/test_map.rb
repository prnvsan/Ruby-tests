...
class TestMap < Test::Unit::TestCase
    ...
    def test_gothon_game_map()
        assert_equal(Map::MODIFIED_DEATH, Map::START.go('shoot!'))
        assert_equal(Map::MODIFIED_DEATH, Map::START.go('dodge!'))

        room = Map::START.go('tell a joke')
        assert_equal(Map::LASER_WEAPON_ARMORY, room)

        assert_equal(Map::MODIFIED_DEATH,Map::LASER_WEAPON_ARMORY.go("*"))

        assert_equal(Map::THE_BRIDGE,Map::LASER_WEAPON_ARMORY.go(Map::CODE))

        assert_equal(Map::MODIFIED_DEATH,Map::THE_BRIDGE.go("throw the bomb"))

        assert_equal(Map::ESCAPE_POD,Map::THE_BRIDGE.go("slowly place the bomb"))

        assert_equal(Map::THE_END_LOSER, Map::ESCAPE_POD.go("*"))

        assert_equal(Map::THE_END_WINNER, Map::ESCAPE_POD.go(Map::GOOD_POD))
